...
leftmost element (which represents current group size). If there is a valid way to group the rest of the people
into the specific group sizes want, then the recursive call would return the first such valid set, and we can
then return the combination of the set of original 4 people group and the set of the groups of the other 6 people.
If there is NO valid way to group the other 6, the recursive call would return null, and if that happens we move
on to the next possible group of 4 and repeat the same procedure.
...
let findAnswer = function(people, groupSizes, hash){
  ...
  if(groupSizes.length === 1){
    ...
    set of groups of specific sizes from the rest of the people.
    */
